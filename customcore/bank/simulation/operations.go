...
func SimulateMsgSend(ak types.AccountKeeper, bk keeper.Keeper) simtypes.Operation {
	return func(
		...
		if err != nil {
			// handle error caused by insufficient fees from tax
			if strings.Contains(err.Error(), "insufficient fee") {
				return simtypes.NoOpMsg(types.ModuleName, types.TypeMsgSend, "skip low fee caused by tax"), nil, nil
			}
...
func SimulateMsgMultiSend(ak types.AccountKeeper, bk keeper.Keeper) simtypes.Operation {
	return func(
		...
		if err != nil {
			// handle insufficient fees error caused by tax
			if strings.Contains(err.Error(), "insufficient fee") {
				return simtypes.NoOpMsg(types.ModuleName, types.TypeMsgMultiSend, "skip low fee caused by tax"), nil, nil
			}
			return simtypes.NoOpMsg(types.ModuleName, msg.Type(), "invalid transfers"), nil, err
		}
		return simtypes.NewOperationMsg(msg, true, "", nil), nil, nil
