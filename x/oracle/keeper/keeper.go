...
// GetVoteDelegate returns a delegate address if it exists, given a *Validator* operator address
// Return error if no delegate mapped
func (k Keeper) GetVoteDelegate(ctx sdk.Context, val sdk.ValAddress) (sdk.AccAddress, error) {
	store := ctx.KVStore(k.storeKey)
...
// GetVoteDelegator returns a validator address if it exists, given a *delegate* address.
// Return error if no validator delegator address mapped
func (k Keeper) GetVoteDelegator(ctx sdk.Context, del sdk.AccAddress) (sdk.AccAddress, error) {
	store := ctx.KVStore(k.storeKey)
...
// SetVoterDelegation stores a vote delegation in both directions to the store for easy lookup
func (k Keeper) SetVoteDelegation(ctx sdk.Context, del sdk.AccAddress, val sdk.ValAddress) {
	store := ctx.KVStore(k.storeKey)
